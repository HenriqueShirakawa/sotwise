...
- **Escopo da thread = Order, até 2 por pedido** (`email_threads`, `unique (order_id, kind)`, `kind in ('internal','external')`). PL/Shipment "emprestam" a thread de cada pedido que consolidam: um envio de etapa de PL entra em **todas** as threads dos N pedidos (fan-out em `checklist_step_email_threads`, insert-only, só auditoria); a linha guarda a thread **primária** em `checklist_step_emails.thread_id` (a do próprio pedido, ou — em PL — a do pedido de menor `po_number`, critério determinístico). PL sem pedido vinculado **bloqueia o envio** com mensagem clara ("no order linked yet") — nunca cai num modo "sem thread" silencioso.
...
- **Assunto fixo por pedido: `Order #1637`** (PL/Shipment: `PL #246`), sem a etapa — o Gmail só agrupa quando o assunto é igual (confirmado no spike de 11/09 abaixo). A etapa vai em **destaque no corpo** (`stepLabel` em `checklistStepEmailHtml`, título grande abaixo do assunto). O assunto continua editável no compositor; quem mudar sai da conversa agrupada por conta própria.
...
- **Limitação conhecida:** um envio de PL que consolida N pedidos é UMA mensagem por destinatário, com assunto `PL #246` — o Gmail (que exige assunto igual) não vai agrupá-la com as conversas `Order #176`/`Order #300`, embora os cabeçalhos apontem pra elas (Outlook/Apple Mail agrupam por cabeçalho). Alternativa (não implementada): mandar N mensagens, uma por pedido, cada uma dentro da conversa do seu pedido.
