...
- **Reset de senha (decisão 2026-08-28):** o "Forgot password" passou a seguir **exatamente o mesmo desenho do convite** — `admin.auth.admin.generateLink({ type: "recovery" })` emite o token sem enviar nada, o app monta `{origin}/auth/callback?token_hash=<hashed_token>&type=recovery&next=/update-password` e despacha pelo Resend (`lib/email/reset-password.ts`). Antes usava `resetPasswordForEmail`, que depende do SMTP do Supabase (bloqueado) e monta o link a partir do **Site URL** — que segue em `localhost:3000` e não temos permissão para alterar. Na prática o reset estava **quebrado em produção**: e-mail não saía e, se saísse, o link não voltava para o app. Consequência boa do contorno: **nenhum** e-mail do app depende de config no painel do Supabase. Por privacidade, e-mail não cadastrado continua devolvendo a mesma mensagem neutra (o generateLink falha em silêncio) — ver a ressalva em 3.12.1.
...
> 🔄 **Mudança (2026-08-24) — a cascata "criar Order → semear as 10 etapas da fase Order" passou a viver no BANCO.** Antes, o seed das 10 linhas em `order_checklist_steps` era feito só em código, dentro da action `createOrder` (`app/(dashboard)/orders/actions.ts`). Qualquer criação de order fora desse caminho nascia **sem checklist** e a tela de detalhe abria com _"No checklist steps for this order."_. Agora um trigger `AFTER INSERT ON orders` (`trg_orders_seed_checklist`, migration `20260824120000`) semeia as 10 etapas para **todo** INSERT de order — a action, a **via inbound do GSS** (ver abaixo) e SQL manual. O seed em código foi removido da action. Motivo: o GSS passou a **criar orders** mandando um `POST /api/gss/orders` (primeira via _push_ da integração, oposta ao _pull_ das bibliotecas); um insert direto na tabela `orders` por esse caminho pularia a cascata se ela continuasse só em código.

> 📥 **Via inbound GSS → Order (2026-08-24).** `POST /api/gss/orders`, autenticado por `Authorization: Bearer $GSS_INBOUND_SECRET`. O GSS manda: `gss_id` (chave natural do pedido no GSS — torna o POST **idempotente**, retry não duplica; gravado em `orders.gss_id`, único), `po_number` (o **GSS** decide o número — unique no banco, colisão responde **409**), e as FKs por `gss_id` da biblioteca (`client_gss_id`, `order_type_gss_id`, `business_unit_gss_id`, `exporter_gss_id`), que o endpoint traduz para o UUID interno via a coluna `gss_id` das libs (gss_id inexistente → **400**). `schedule_requested`, `client_reference` e `date_po` são opcionais (`date_po` default = hoje **só na criação**). **Leader e Requester** apontam para `profiles` (usuários do SOTWISE, sem `gss_id`); como profiles não têm `gss_id`, o GSS os identifica por **e-mail** — `leader_email`/`requester_email` (opcionais), resolvidos para o id do profile pela função `public.profile_id_by_email` (case-insensitive; migration `20260825130000`). E-mail que não casa nenhum usuário → **400**.

> 📤 **Via de leitura SOTWISE → GSS (2026-09-01).** `GET /api/gss/orders`, **mesmo path e mesmo secret** do push (`GSS_INBOUND_SECRET`) — a integração de orders deixou de ser mão única. Motivo: o GSS escrevia a order e não tinha como saber o que aconteceu com ela depois (status, lote, checklist), tudo estado que só existe no SOTWISE. A resposta é **sempre uma lista** (`{ data, pagination }`), inclusive filtrando por uma order só — `?gss_id=` devolve 0 ou 1 item, e a forma não muda com o filtro (simplifica o parse do lado do GSS; **não existe 404** aqui, filtro sem casamento é `200` com `data: []`). Filtros: `gss_id`, `po_number`, `status`, `updated_since` (ISO 8601 com fuso), `order` (`asc`|`desc` por `updated_at`, default `desc`), `limit` (1–200, default 50), `offset`. Ordenação é `updated_at` **+ `id`** — o `id` desempata para nenhuma order pular ou repetir entre páginas quando os timestamps empatam. **Sincronização incremental** = `updated_since` + `order=asc`, paginando até `returned < limit`. Cada biblioteca sai como `{ id, name, gss_id }`: o GSS reconcilia pelo **mesmo `gss_id`** que usa para escrever, sem conhecer UUID interno. **Leader/Requester saem como `{ id, name }`, não e-mail** — o e-mail mora em `auth.users`, que o PostgREST não expõe, e devolvê-lo exigiria outra função SECURITY DEFINER (fazer só se o GSS precisar casar por e-mail). Blocos pesados são **opt-in** por `include=items,checklist`: `items` traz as linhas Factory×Category **com o lote atribuído no SOTWISE** (o que o GSS não tem de outro jeito) e `checklist` as 10 etapas da fase Order na ordem canônica (Pre-loading e Shipment **não** entram). Só código — **sem migration**. Código: `domain/orders/gss-read.ts`; doc para o time do GSS: `docs/SOTWISE-API-para-GSS.md` §1.5.

> 🔧 **Auth unificada, path mudou (2026-09-10).** O secret dedicado
> `GSS_INBOUND_SECRET` citado nos dois blocos acima foi aposentado: `POST`/`GET`
> agora vivem em `/api/orders` (não mais `/api/gss/orders`) e autenticam por
> `Authorization: Bearer $API_TOKEN` — o mesmo mecanismo que a API de
> Bibliotecas já usava (`requireApiSession()`, `lib/api-auth.ts`), que também
> aceita sessão de cookie de um usuário com permissão na feature `orders`.
> Trocado direto, sem período de transição, porque o GSS ainda não tinha
> implementado a chamada de Orders do lado deles (só as Bibliotecas estavam em
> uso) — não havia tráfego de produção em risco. Resto do contrato (payload,
> idempotência por `gss_id`, filtros do GET) não muda. Doc pro time do GSS:
> `docs/SOTWISE-API-para-GSS.md` §1.
>
> 🔧 **Update é PARCIAL, não espelho (2026-08-25).** Como no GSS a order é criada e depois preenchida em **momentos distintos**, o reenvio só toca as colunas do cabeçalho que **vierem** no payload; campo **omitido não mexe** na coluna (um `null` explícito é que limpa). Por isso `po_number` virou **obrigatório só na criação** — no reenvio pode ser omitido (a order é identificada pelo `gss_id`). Assim dá para mandar só `{ gss_id, items }` depois sem zerar client/leader/etc.

Ainda no payload: **`items[]`** (opcional) traz as linhas **Factory×Category** (`order_factory_category`): cada item é `{ supplier_category_gss_id, ship_requirement }` — o endpoint deriva fábrica+categoria do `supplier_category_gss_id` via `factory_products` (gss_id inexistente → **400**), grava a `ship_requirement` (obrigatória por linha) e deixa o **lote (`batch_id`) NULL** (o usuário atribui depois no SOTWISE). É **não destrutivo**: reenvio só ADICIONA pares (factory, category) novos; pares já existentes na order não são recriados nem têm lote/ship_requirement sobrescritos (protege o trabalho de lote do usuário). Respostas: `201` criada / `200` atualizada / `400` payload / FK / e-mail / supplier-category inválido / `401` sem secret / `409` colisão de `po_number`.

> 👁️ **Visibilidade da lista Orders (2026-08-25): order NOVA só aparece quando tem ≥1 linha Factory×Category** (`order_factory_category`). Vale só **a partir do corte** `NEW_ORDER_CUTOFF = 2026-08-25T00:00:00Z` (constante em `app/(dashboard)/orders/page.tsx`): orders criadas antes do corte são **grandfathered** (sempre visíveis — não escondemos as ~388 orders migradas que nasceram sem F×C). O filtro é em código: `created_at < corte || tem F×C`. **Realtime:** a via inbound (`/api/orders`) dispara `broadcastOrderStatusPing` após criar/atualizar, então a order nova **surge na lista sem F5** assim que ganha a 1ª F×C (o client faz `router.refresh()` no ping — `lib/use-orders-realtime.ts`).

> ⚠️ **A API de leitura do GSS (`/orders/`) NÃO expõe as Factory×Category nem produtos** (schema confirmado em 25/08/2026: a Order de lá é só cabeçalho; e o nosso usuário técnico ainda recebe `403` em `/orders/`). Logo, essas linhas só chegam ao SOTWISE **pelo push** (`items[]`). O GSS identifica usuário por **`username`** (`leader_username`/`requester_username`), não e-mail — a via por e-mail depende de o GSS incluir o e-mail no push. A Order do GSS tem **id inteiro** e **não tem `po_number`**.
...
##### Fase 2.1 — 3 User Stories do cliente (decisão 2026-09-09)

O cliente trouxe 3 User Stories formais (idioma automático, destinatários revisáveis, histórico auditável). Os pontos que não batiam com o schema real foram substituídos por decisões adaptadas ao sistema:

- **Destinatário passou a ter uma ÂNCORA obrigatória: o `responsible_id` da própria etapa** (campo "Responsible" já existente e editável em `order_checklist_steps`/`pre_loading_checklist_steps`). Superou a regra anterior ("destinatários selecionáveis = todos os usuários ativos, não restrito a quem já está ligado ao pedido") — o responsible entra sempre pré-selecionado no compositor e **não pode ser removido** (`MultiSearchSelect` ganhou `lockedIds`); o botão "Send email" fica desabilitado se a etapa não tem `responsible_id`. Outros destinatários (todos os usuários ativos, regra antiga) continuam podendo ser adicionados por cima.
- **Falha de envio (total ou parcial) não fecha mais o modal** — só sucesso 100%. Os dados (destinatários/assunto/corpo) continuam preenchidos pra tentar de novo sem redigitar.
- **Idioma automático do template** (`lib/email/checklist-step.ts`, param `language: "pt-BR" | "en" | "zh"`): resolvido por `clients.language` (coluna nova; hoje sempre nula — **o customer do GSS não manda idioma**, confirmado batendo no endpoint `/core/customer/` ao vivo em 09/09/2026, apesar do pedido inicial do cliente dizer que mandava) → fallback `country_language_defaults` (tabela nova, seedada só Brasil→pt-BR/China→zh, sem tela de admin) pelo `country_id` do cliente → default global `'en'`. Nunca bloqueia o envio. Só o CHROME do e-mail troca de idioma (rótulos fixos, botão, rodapé) — `subject`/`body` continuam texto livre, sem tradução automática nem sistema de variáveis (`{{nome_cliente}}` etc. não foi construído).
- **`checklist_step_emails` ganhou `status`** (`success`/`partial`/`failed`, computado uma vez no envio e congelado — nunca recalculado a partir de `recipients` na leitura) **e `language`** (o idioma efetivamente usado naquele envio). Ambas nullable — aditivas, linhas de antes de 09/09/2026 ficam `null`.
- **Imutabilidade de verdade, não só convenção de código:** `revoke update, delete on checklist_step_emails from anon, authenticated, service_role` — antes só não existia `CREATE POLICY`, mas o `service_role` (usado por toda a DAL) contornava RLS livremente.
...
- **Corpo padrão só da etapa "pi"** por enquanto (`lib/email/step-templates.ts` — mapa por `ChecklistStep`, hoje só uma entrada preenchida). Texto continua 100% editável — é ponto de partida, não trava nada. ⚠️ **Revisado em 10/09** — ver bullets abaixo, a versão bilíngue original foi removida e o corpo padrão passou a cobrir as 24 etapas.

> ⚠️ **Revisão 10/09/2026 — corpo virou só inglês + nomes de verdade.** O usuário reportou erro na tradução automática (pt-BR/zh) e pediu **só a versão em inglês** — removida a cascata de idioma do corpo padrão (`resolveStepEmailLanguage`/`STEP_EMAIL_TEMPLATES[lang]` saíram; o chrome do e-mail — rótulos, botão, rodapé — continua traduzido normalmente, isso é outro mecanismo, intocado). Em troca, `[Customer Name]`/`[Your Name]`/`[Company Name]` viram **substituição automática toda vez que o compositor abre** (`loadStepEmailDefaults` em `lib/checklist-email-actions.ts`): Customer Name e Company Name recebem o nome do(s) cliente(s) da etapa (pedido explícito do usuário — `[Company Name]` não é a empresa de quem envia), Your Name recebe o nome de quem está logado compondo. Sem resolver (etapa sem cliente vinculado), o colchete original fica, editável à mão como antes.
...
- **Exige "Receiving" ativado no domínio de `EMAIL_FROM` no Resend** (hoje só envia) + um registro MX novo no DNS (Cloudflare do cliente) + um Webhook cadastrado no Resend (evento `email.received`) apontando para `/api/webhooks/resend`, com o Signing Secret em `RESEND_WEBHOOK_SECRET`. Passos manuais, fora do código — ver `.env.example`.
- **Matching por endereço único por envio, não por header de thread:** todo `sendEmail` de uma linha de `checklist_step_emails` leva `Reply-To: reply+<id-da-linha>@<domínio>` — o próprio `id` da linha é o token (gerado com `randomUUID()` **antes** do envio e inserido explícito no `insert` final, pra existir a tempo de compor o Reply-To). Autenticidade não depende de casar o remetente — o UUID já funciona como capability token; o "From" só resolve `from_user_id`/nome pra exibição, comparando contra o `recipients[].email` congelado na linha pai.
- **Tabelas novas** (migration `20260909130000`): `checklist_step_email_replies` (insert-only, `revoke update/delete` igual `checklist_step_emails` — uma resposta NUNCA vira UPDATE na linha original, que segue imutável) e `checklist_step_email_reply_recipients` (mesmo padrão de `message_recipients`: uma linha por destinatário a notificar, `read_at` nulo = não lida). Notificados = `recipients[].user_id` da linha pai + `sender_id`, menos quem respondeu.
- **Webhook do Resend só traz metadados** (`from`/`to`/`subject`/`email_id`) — o corpo exige uma segunda chamada (`fetchReceivedEmail`, `GET /emails/receiving/:id`). Assinatura verificada à mão (Svix HMAC-SHA256 sobre `svix-id.svix-timestamp.body`, `lib/email/verify-resend-webhook.ts`) — sem adicionar a dependência `svix`, mesmo espírito de `lib/email/resend.ts` já evitar o SDK do Resend. Idempotente por `provider_message_id` (`data.email_id`) — retry do Resend não duplica.
- **A UI nunca renderiza o HTML da resposta cru** (conteúdo externo não confiável) — só `body_text` (gerado a partir do `html` no servidor quando o provedor não manda `text`). `body_html` fica guardado só para auditoria.
...
##### E-mail em "thread" por Order (decisão 2026-09-10, Fase 2 em 2026-09-11)

Pedido do cliente (10/09): cada "Send email" de etapa chegava na caixa de entrada como conversa isolada. Ele quer **um único e-mail por pedido, com cada etapa chegando como Reply** — "basicamente 1 e-mail do pedido 1637 com reply de todos os checklists" — diferenciando só **quando tem cliente e quando não tem**. A tela não muda: compositor, "Emails sent" e histórico continuam por etapa; a thread é metadado por trás.

- **Escopo da thread = Order, até 2 por pedido** (`email_threads`, `unique (order_id, kind)`, `kind in ('internal','external')`). PL sem pedido vinculado **bloqueia o envio** com mensagem clara ("no order linked yet") — nunca cai num modo "sem thread" silencioso. ⚠️ **Superado em 16/09/2026 (ver bullet "Fan-out por Order" mais abaixo)**: até então, PL/Shipment "emprestavam" a thread de UM pedido primário só (a do menor `po_number`) pra todo o envio — hoje cada pedido consolidado recebe sua própria cópia, na sua própria thread.
- **Etapa de thread `external` = e-mail só de leitura/resposta (decisão 11/09/2026):** numa etapa da conversa do cliente, **nenhum** destinatário recebe os campos internos (Estimated date/Responsible/Completed on/Signed by) nem o botão "Go to" — nem os internos que estiverem no mesmo envio. A regra antiga (quem tem papel `client` nunca recebe esses campos) continua valendo como piso, inclusive numa etapa `internal`. Ou seja, a variante do HTML é `externalThread || recipienteÉCliente`. Reflete no preview do compositor (etapa `external` só oferece a aba de cliente).
- **Qual thread cada etapa alimenta é decidido pela ETAPA, não pelo destinatário** (`lib/email/step-thread-kind.ts`). Decisão do usuário em 11/09: por enquanto só **Deposit Payment** e **Balance Payment** são `external`; todo o resto `internal`. O mapeamento definitivo vai ser dado pelo cliente (AGK) — é só trocar valores nesse arquivo. Eixo independente de "cliente vs. interno" de cada destinatário (variante do HTML, por papel) — não confundir.
- ⚠️ **Atualização 16/09/2026:** o usuário confirmou que o mapeamento pendente cobre também as etapas do **Pre-loading e do Shipment** — nenhuma delas manda e-mail pro cliente ainda ("ainda vamos manter TODOS em equipe interna"); ele vai passar a lista final depois. Achado à parte: trocar `internal`→`external` em `step-thread-kind.ts` **não impede sozinho** um cliente de ser escolhido como destinatário — `loadStepRecipientOptions` (`lib/checklist-email-actions.ts`) devolve todo profile ativo sem filtrar por papel nem por etapa/feature. Hoje depende de quem compõe não selecionar cliente nessas etapas; se a restrição precisar valer de verdade, é ali que entra o filtro, não só no mapa de thread.
- **Assunto fixo por pedido: `Order #1637`**, sem a etapa — o Gmail só agrupa quando o assunto é igual (confirmado no spike de 11/09 abaixo). A etapa vai em **destaque no corpo** (`stepLabel` em `checklistStepEmailHtml`, título grande abaixo do assunto). O assunto digitado no compositor (`PL #246` pra PL/Shipment) continua editável e é o que aparece NO CORPO. ⚠️ **Superado em 22/09/2026**: entre 16/09 e 22/09 o ENVELOPE de uma cópia de PL/Shipment usava `Order #<po_number>` forçado (ver "Fan-out por Order" abaixo); desde que PL/Shipment ganhou thread própria, o envelope voltou a ser sempre o assunto digitado no compositor, igual à Order (ver "PL/Shipment ganham thread própria" mais abaixo).
- **Mecanismo (Fase 2):** cada envio novo leva `In-Reply-To: <Message-ID da âncora da thread primária>` e `References: <âncoras de todas as threads atingidas + Message-ID mais recente da primária>`; o Resend aceita `headers` no POST e devolve o Message-ID real via `GET /emails/:id` logo após (best-effort, poucas tentativas — `sendEmail` em `lib/email/resend.ts`). **Âncora = primeira linha da thread COM Message-ID** (`promoteAnchorIfMissing`, guard `anchor_message_id is null`); e-mails de antes da Fase 2 (sem Message-ID capturado) ficam fora da conversa agrupada — **nada migrado retroativamente**, decisão do usuário. Cada destinatário recebe mensagem própria (Message-ID próprio): a linha guarda o primeiro entregue em `message_id` e todos em `recipients[].message_id`.
- **Reply-To passa a ser da thread**: `reply+<email_threads.id>@<domínio>`. O webhook (`/api/webhooks/resend`) resolve o token nesta ordem: id de thread → casa `In-Reply-To`/`References` do e-mail recebido contra o `message_id` de cada linha da thread (`attribution = 'message_id'`, a etapa exata); sem cabeçalho que bata (forward, cliente que descarta) → linha mais recente da thread, `'fallback'`, e o card da resposta na tela avisa "Exact step not confirmed"; token que não é thread → id de linha, caminho antigo (`'direct'`) — envios antigos continuam funcionando. Coluna `attribution` (migration `20260911120000`, nullable; respostas anteriores ficam null).
- **Spike (Fase 0, 11/09/2026):** 3 e-mails reais via Resend com `In-Reply-To`/`References` e mesmo assunto chegaram ao Gmail do usuário como **uma conversa única** (contador 3). Sem esse teste a Fase 2 não seria construída.
- **Reply de verdade, não só cabeçalho (feedback do usuário 11/09, comparando com uma conversa humana no Gmail):** envio seguinte vai com `Re:` no assunto (só no SMTP — a linha guarda o assunto como digitado) e com o **histórico anterior da thread citado e aninhado no rodapé** ("Em <data>, <fulano> escreveu:" + etapa + texto, cada nível citando o anterior, classe `gmail_quote` pra colapsar atrás do "..."), todas as mensagens da thread (`loadQuotedHistory`; `peekQuotedHistory` pro preview, sem criar thread). Sem isso, mesmo agrupado, cada reply "parecia" e-mail avulso.
- **Todos os destinatários no "To" (decisão 11/09/2026):** o envio deixou de ser uma mensagem por pessoa e passou a ser **uma por grupo** — todos os internos numa mensagem, todos os clientes em outra, cada grupo no próprio "To". Motivo: o usuário quer que, quando alguém responder, **os outros participantes recebam a resposta na caixa de entrada deles também** — com todos no "To", um "Responder a todos" entrega direto a todos e ainda passa pelo SOTWISE (o `Reply-To` da thread continua lá). Consequências aceitas explicitamente: (a) os destinatários de um mesmo grupo passam a ver o endereço uns dos outros; (b) um "Responder" simples (só pro remetente) continua chegando só ao SOTWISE — quem quiser que os colegas vejam precisa usar "Responder a todos". A separação equipe × cliente **não** foi afrouxada: são duas mensagens distintas porque cliente nunca pode ver campos internos/botão "Go to" — e assim um grupo também não enxerga os endereços do outro. Alternativa descartada pelo usuário: o SOTWISE reenviar cada resposta a cada participante (funcionaria com "Responder" simples e sem ninguém ver o e-mail de ninguém).
- **Destinatário avulso (decisão 11/09/2026):** o compositor ganhou um campo pequeno abaixo do seletor de pessoas pra digitar **e-mail de quem não tem cadastro no SOTWISE** (Enter/vírgula/botão "Add" adicionam; chips removíveis; validação de formato no cliente e no `sendSchema`, máx. 20). Esse destinatário entra na linha de histórico como qualquer outro, com `recipients[].user_id = null` — por isso o tipo `StepEmailRecipient.user_id` virou `UUID | null` (chips passam a usar o e-mail como chave de lista, e o fan-out de notificação do webhook ignora os nulos: não há usuário a notificar dentro do app). **Sempre recebe a versão limpa** (sem campos internos/botão "Acessar"): sem perfil, não dá pra checar papel, então o seguro é tratar como externo. O e-mail digitado que coincidir com o de um usuário já selecionado é descartado (não duplica). O `MultiSearchSelect` NÃO foi tocado — é compartilhado com 5 telas sem relação com e-mail.
- ⚠️ **Limitação conhecida, RESOLVIDA em 16/09/2026** — ver bullet "Fan-out por Order" abaixo (era: um envio de PL que consolida N pedidos era UMA mensagem por destinatário, com assunto `PL #246`; o Gmail não agrupava com as conversas `Order #176`/`Order #300`).
- **Ainda não feito (Fase 3):** destinatário avulso (e-mail sem cadastro no SOTWISE) — independente das fases de thread.
- Código: `lib/email/threads.ts` (resolução/fan-out/âncora/cabeçalhos), `lib/email/step-thread-kind.ts`, `lib/checklist-email-actions.ts` (`sendStepEmail`), `lib/email/resend.ts`, `app/api/webhooks/resend/route.ts`; migrations `20260910120000_email_threads` e `20260911120000_email_reply_attribution`.

##### Fan-out por Order — cada pedido consolidado ganha sua própria resposta (decisão 2026-09-16)

⚠️ **Superado em 22/09/2026 (ver seção "PL/Shipment ganham thread própria" mais abaixo)**: até então, PL/Shipment não tinha thread própria nenhuma — "emprestava" a de CADA Order consolidada (fan-out descrito abaixo). Hoje PL/Shipment tem sua(s) própria(s) thread(s), independente de qualquer Order.

Bug reportado ao vivo: o PL #1447 (consolida os pedidos 1305.04, 1305.03, 1637.01) resolvia sempre o pedido 1637 como "primário", então TODA etapa do PL (Consolidation Point, City, ...) chegava no Gmail como reply dentro da conversa **do pedido 1637**, citando no rodapé o histórico de etapas do 1637 ("Order", "PO") que não tinham nada a ver com o PL. Os outros 2 pedidos não recebiam nada visível. Pedido explícito do usuário: **"eu QUERO que ela continue a partir de ORDERS... quando eu mandar email de consolidation point que contenha o 1637, ele continue na thread de 1637 do order"** — vale pra QUALQUER etapa depois da fase Order (Pre-loading e Shipment; não é específico de nenhuma etapa), pra destinatário interno e pra cliente.

- **Cada Order consolidado recebe sua PRÓPRIA cópia do e-mail**, respondendo de verdade dentro da conversa daquele Order — não mais "pegando carona" numa thread primária única. `sendStepEmail` (`lib/checklist-email-actions.ts`) passou a renderizar o HTML (incluindo o histórico citado, `loadQuotedHistory`) uma vez POR THREAD de Order atingida, e a chamar `insertEmailRow` uma vez por Order — não muda `lib/email/threads.ts` (as funções já suportavam thread única, só o chamador agrupava N delas num envio só).
- **Destinatário "plain" (cliente)** recebe uma cópia em cada Order que o SEU cliente de fato tem no owner (`threadsForLanguage`, já existia pra Fase multi-idioma); **destinatário interno** recebe uma cópia em TODOS os Orders do owner (não é ligado a nenhum cliente específico).
- **Assunto do ENVELOPE muda pra `Order #<po_number>`** do Order daquela cópia (aplica-se mesmo quando o owner só tem 1 Order — sem essa uniformidade, o bug original continuaria no caso mais comum, que é PL de 1 Order). O rótulo visível NO CORPO continua sendo o assunto digitado no compositor (ex.: "PL #1447") — só o envelope SMTP muda, não o `checklist_step_emails.subject` gravado.
- **Efeito colateral positivo:** antes, `promoteAnchorIfMissing` promovia a âncora dos 3 Orders com o Message-ID do MESMO e-mail físico — que só o Order primário de fato recebeu. Com o fan-out por Order (`fanoutThreads: [thread]`, 1 elemento), cada Order só tem sua âncora promovida por um e-mail que ele mesmo recebeu.
- **Efeito esperado:** o número de e-mails/linhas de "Emails sent" por envio passa a ser maior pra PL/Shipment com 2+ Orders (1 por Order atingido, não 1-2 no total). Pra Order (`owner.kind === "order"`) e PL/Shipment de 1 Order só, nenhum loop novo roda mais de 1 vez — comportamento idêntico ao de antes, exceto o assunto do envelope.
- **Histórico ganhou o Order de cada linha** — `StepEmailRow.order_po_number`/`EmailListRow.orderPoNumber` (resolvidos via `thread_id` → `email_threads.order_id` → `orders.po_number`, só pra owner que não é `"order"`), exibido como chip "Order #..." em `EmailHistoryCard` (por etapa) e na coluna Record de `/emails` — sem isso, N linhas do mesmo envio (mesmo assunto, mesmo corpo) pareceriam duplicadas.
- **Preview não mudou de lógica** — `previewStepEmail`/`peekQuotedHistory` continuam mostrando o histórico de UM Order representante (o de menor `po_number`); só ganhou um aviso no compositor deixando claro que cada Order consolidado recebe sua própria cópia.

##### PL/Shipment ganham thread própria, independente de qualquer Order (decisão 2026-09-22)

Mudança de requisito do cliente: até aqui, `email_threads` era estritamente de uma Order (`order_id`, `unique(order_id, kind)`) — o fan-out por Order acima existia porque PL/Shipment não tinha CONVERSA PRÓPRIA nenhuma, só "pegava carona" na de cada Order consolidada. Pedido do usuário (verbatim): **"ORDERS são emails apenas dos checklists de emails [da Order]. Quando estivermos em PL vamos mandar por PL... seria meio que ter 4 threads agora: 2 orders, 2 PL/ship, duas internas e duas externas de cada."**

- **Owner polimórfico**: `email_threads` trocou `order_id` por `owner_type` (`'order' | 'pre_loading'`) + `owner_id` — mesmo padrão já usado em `messages`/`message_entity`. Pre-loading e Shipment continuam compartilhando `owner_type='pre_loading'` (mesmo checklist, `StepOwner.kind==='pre_loading'`) — não existe um 3º owner `'shipment'`. Migration `20260922120000_email_threads_owner_columns` (Fase A, aditiva — mantém `order_id` redundante até o deploy confirmado; Fase B, cleanup, remove a coluna depois).
- **`kind='internal'` é sempre 1 thread só pro owner inteiro** — Order (como sempre foi) ou Pre-loading/Shipment inteiro (novo: não fragmenta mais por Order consolidada). Destinatário interno não é client-scoped, não há por que dividir a conversa da equipe.
- **`kind='external'` de Pre-loading/Shipment é 1 thread POR CLIENTE distinto** consolidado no PL (`client_id` na linha) — nunca funde 2 clientes reais na mesma conversa (vazamento de visibilidade entre empresas). Exemplo do usuário: PL-1212 com lotes da Amacom E da AGK manda 2 e-mails/threads distintas, uma por cliente; 3 clientes seriam 3 threads. Orders sem `client_id` caem juntas no balde `client_id null` (nunca junto de um cliente real). `owner_type='order'` nunca divide por cliente — a Order já tem 1 só.
- **Destinatário `external` cujo `client_id` não bate com nenhum cliente que o PL de fato consolida agora é REJEITADO** com erro pedindo pra reabrir o compositor — nunca cai silenciosamente numa thread errada (é o vazamento que este modelo existe pra evitar). Mesmo espírito do guard já existente pra drift de idioma.
- **Assunto do ENVELOPE volta a ser sempre o assunto digitado no compositor** (`smtpSubjectForThread` perdeu o branch que forçava `Order #<po_number>` pra owner Pre-loading/Shipment) — sem mais "pegar carona", não há razão pra imitar o assunto de uma Order. `bodySubjectByOrderId` (sufixo `— Order #<po_number>` no corpo) também saiu: sem fan-out por Order, não sobra mais "qual cópia é essa" pra desambiguar.
- **Histórico mostra o CLIENTE da thread, não mais a Order** — `StepEmailRow.thread_client_name`/`EmailListRow.threadClientName` (via `thread_id` → `email_threads.client_id` → `clients.name`), exibido sem prefixo "Order #" em `EmailHistoryCard` e na coluna Record de `/emails` (ex.: "PL 246 · Amacom"). Só não-nulo pra thread `external` de PL/Shipment dividida por cliente.
- **`[Customer Name]` na thread `internal` de PL com 2+ clientes reais fica sem substituição** (`customerNameForThread` devolve `null` — o rascunho mantém o colchete literal se o usuário não reescreveu à mão). Aceito: destinatário de step-email hoje é sempre `client`-role (`loadStepRecipientOptions`), mas o `kind` (internal/external) é decidido pela ETAPA, não pelo destinatário — hoje 100% das etapas são `internal`, então essa é a thread que domina o tráfego real; quando a AGK definir o mapeamento internal/external de verdade, o caso comum passa a ser `external` (client_id sempre resolvido, sem essa lacuna).
- Código: `lib/email/threads.ts` (`resolveOwnerThreads`, `findOrCreateThread`, `ThreadOwnerKey`), `lib/checklist-email-actions.ts` (`sendStepEmail`, `customerNameForThread`, `threadByClientId`), `lib/checklist-emails-list-actions.ts`, `domain/client/notifications.ts`; migrations `20260922120000_email_threads_owner_columns` + `20260922130000_email_threads_order_id_nullable` (correção: a Fase A esqueceu o `not null` de `order_id`, todo insert novo falhava).

##### Compositor de PL/Shipment com uma aba-pasta por cliente (decisão 2026-09-22)

Pedido do usuário: um PL com 5 clientes abre o "Send email" com 5 abas (tipo pastas); manda pra AGK, o pop-up **não fecha** — limpa e passa pra Amacom, e assim até completar os 5. Resultado esperado (verbatim, confirmado): **"1 conversa com a AGK e 1 com a Amacom, separadas. Cada uma recebe o próprio e-mail, com o nome dela."**

- **Cada aba = a conversa PRÓPRIA daquele cliente, sempre** — independente do mapeamento internal/external da etapa (`step-thread-kind.ts`, que segue todo `internal` até a AGK mandar a lista). Tecnicamente é a mesma thread `external` por cliente (`client_id` preenchido) da seção acima: o envio passa `client_id` e `resolveOwnerThreads` usa `{pre_loading, client_id}` com `kind='external'`. Quando a lista da AGK chegar, etapas externas e abas caem na MESMA conversa. O `kind` da etapa continua decidindo só a variante do HTML (equipe recebe campos internos/"Go to" enquanto a etapa for `internal`).
- **Idioma e nome são do cliente da aba** (`resolveClientLanguageGroups` com escopo): 1 grupo só, rascunho já com o nome dele ("Prezado(a) AGK" / "尊敬的 Amacom") — sem colchete `[Customer Name]` sobrando. A equipe no "To" daquela aba recebe o mesmo texto/idioma (WYSIWYG).
- **"To" pré-preenchido com Responsible + Signed by da etapa**; a lista da aba oferece a equipe inteira + os contatos SÓ daquele cliente (`loadStepRecipientOptions(clientId)`). **Contato do cliente é opcional.** Guard no servidor: destinatário `client` de outro cliente é rejeitado, e `client_id` que o PL não consolida mais pede pra reabrir o compositor.
- **Sem aba "Team"** por enquanto (decisão do usuário) — todo envio de PL/Shipment com cliente identificado sai por aba. PL sem cliente nenhum identificado e Order: compositor antigo, sem aba. 1 cliente só: sem barra de abas, mas já usa a conversa própria do cliente.
- Depois do envio a aba ganha ✓ e o modal abre a próxima não enviada; "Close" em vez de "Cancel" quando alguma já foi. Trocar de aba descarta o que foi digitado na atual.
- Código: `components/checklist/step-email-section.tsx` (`openClientTab`, `clientTabs`/`sentClientIds`), `lib/checklist-email-actions.ts` (`client_id` no `sendSchema`, `checkScopeClient`, `loadOwnerClients`, `loadStepTeamIds`), `lib/email/threads.ts` (`scopeClientId` em `resolveOwnerThreads`/`peekQuotedHistory`).

##### Corpo do e-mail traduzido no disparo, pelo país do cliente (decisão 2026-09-11, implementado 14/09)

Pedido do usuário: "o template deve seguir junto com o country de cada cliente; esse template deve ser traduzido na hora do disparo". Supera a decisão de 10/09 de manter o corpo só em inglês — aquela foi reação à v1 **estática** bilíngue (traduções fixas escritas à mão saíram erradas); agora a tradução é **dinâmica, do texto final**, então cobre o template E o que o usuário editou à mão.

- **Idioma = o mesmo que já governa o chrome do e-mail** (`resolveLanguage`: `clients.language` → `country_language_defaults` pelo `clients.country_id` → `en`). Hoje: Brazil → `pt-BR`, China → `zh`, resto → `en` (nada a traduzir). PL/Shipment com vários clientes: o primeiro que resolver, como no chrome.
- **Só a variante do CLIENTE tem o corpo traduzido.** A variante interna (equipe) vai com o texto exatamente como foi escrito no compositor — a equipe escreve em inglês e é assim que ela relê a própria mensagem. Portanto: etapa `external` (todo mundo recebe a versão de cliente) → todos recebem traduzido; etapa `internal` com cliente + equipe no mesmo envio → cliente traduzido, equipe original.
- **Mecanismo** (`lib/email/translate.ts`): chamada ao Claude (`claude-opus-5`, effort `low`, mesma `ANTHROPIC_API_KEY` do Copilot) com instruções de preservar nomes, números de PO/PL, datas, valores, URLs, placeholders `[...]`, quebras de linha e jargão que o time usa em inglês (PI, PO, Booking, ETD, BL); se o texto já estiver no idioma alvo, devolve igual. Roda dentro de `renderStepEmailHtmls`, que serve **preview e envio** — o "Client view" do preview mostra a tradução de verdade; um cache em memória por (idioma, texto) faz o "Confirm & send" reaproveitar a tradução que o usuário acabou de ver (mesma instância).
- **Nunca bloqueia o envio** (mesma regra RN03 do idioma do chrome): sem chave, sem crédito, timeout (60s) ou resposta vazia → o cliente recebe o inglês, e o preview mostra um aviso âmbar com o motivo ("Could not translate the message to Brazilian Portuguese — the client will receive it in English. (...)"). Com tradução OK, o preview mostra uma nota discreta "Client view translated to Brazilian Portuguese (from the client's country). The internal view keeps your original text."
- **O que NÃO muda:** assunto (`Order #1637`, não é texto), título da etapa (`STEP_LABELS`, jargão), histórico citado no rodapé (vem do `body` gravado, em inglês) e a linha em `checklist_step_emails` (guarda o `body` original + `language`; a tradução não é persistida — decisão de escopo, dá pra adicionar coluna depois se o histórico precisar mostrar o que o cliente recebeu).
- ⚠️ **Verificado local em 14/09 só até o aviso**: fluxo inteiro rodou (pt-BR resolvido pelo país do AGK, aviso âmbar exibido, inglês como fallback), mas a chave da Anthropic do `.env.local` estava **sem crédito** ("Your credit balance is too low") — a tradução em si ainda não foi vista. Mesma chave vale pro Copilot (Lapha): sem crédito, os dois param. Assim que a conta tiver crédito, o preview de qualquer etapa com cliente brasileiro mostra o resultado, sem mexer em código.

##### Tradução dinâmica abandonada; corpo estático por idioma + override manual de cliente (decisão 2026-09-14/15)

A v2 (seção acima) nunca chegou a rodar em produção com crédito de verdade. Decisão do usuário em 14/09: abandonar a tradução dinâmica via Claude — dependia de crédito de API, que não é garantido — e usar sempre o **template padrão da etapa** (`lib/email/step-templates.ts`) quando o idioma resolvido do cliente é diferente de inglês, ignorando o que foi editado no compositor. `lib/email/translate.ts` foi apagado (commit `5615a4f`). No mesmo dia, `clients.language` (já existia no banco desde a Fase 2.1, sem UI) ganhou um campo no formulário de Clients — "Checklist e-mail language" (Use country default / Portuguese / English / Chinese) — pra override manual por cliente, acima do fallback por país (commit `2ba7a81`).

- ⚠️ **Gap descoberto 15/09 (reportado pelo usuário, print do compositor com cliente pt-BR):** `5615a4f` trocou PARA o "template padrão da etapa", mas esse arquivo só tinha texto em **inglês** — nunca existiu conteúdo em pt-BR/zh depois que a v1 bilíngue foi removida em 10/09. Resultado prático: nenhum cliente não-inglês recebia corpo traduzido, mesmo com `clients.language` setado à mão em `2ba7a81` — só o chrome (rótulos "Responsável"/"Acessar", rodapé, ver `lib/email/checklist-step.ts`) mudava de idioma; o parágrafo em si saía sempre em inglês, contradizendo o aviso âmbar do compositor ("They will receive the standard template for this step").
- **Corrigido 15/09:** `lib/email/step-templates.ts` ganhou as 24 etapas também em pt-BR e zh (`PT_BR_TEMPLATES`/`ZH_TEMPLATES`), escritas à mão (não tradução automática — mesmo espírito da decisão de 14/09). `buildDefaultStepBody(step, vars, language)` ganhou um 3º parâmetro (default `'en'`); só `renderStepEmailHtmls` passa o idioma resolvido do cliente — o compositor (`StepEmailSection`) continua sempre editando/mostrando a versão em inglês como rascunho.
...
##### v5 — idioma por CLIENTE em PL/Shipment multi-cliente, com abas (decisão 2026-09-15)

A v4 (acima) corrigiu o CONTEÚDO (templates reais em pt-BR/zh), mas um Pre-loading/Shipment que consolida clientes de idiomas diferentes ainda resolvia **1 idioma só pra todo mundo** — "o primeiro cliente que resolver" (`resolveLanguageAndCustomerName`). Reportado pelo usuário num Shipment com cliente zh + outros: pediu (a) manter a thread por Order funcionando, e (b) uma aba por idioma acima da caixa de mensagem.

- **Cada cliente resolve o PRÓPRIO idioma agora** (`resolveClientLanguageGroups`, troca `resolveLanguageAndCustomerName`), agrupado — não mais "o primeiro vale pra todos". Grupo `[0]` é sempre o PRIMÁRIO (desempate determinístico por nome do cliente, não mais ordem arbitrária do Postgres) — absorve Orders órfãos e é o idioma que a equipe interna sempre recebe (mesmo efeito colateral aceito desde a v4).
- **Abas no compositor E no preview, só quando `languages.length > 1`.** Pra Order (sempre 1 cliente) e PL/Shipment de idioma único — a esmagadora maioria dos envios — zero mudança visual, mesma caixa única de sempre. Cada aba é 100% independente e WYSIWYG (mesmo princípio da v4): o texto que estiver nela é o que sai, sem swap.
- **Threads por Order + idioma:** `lib/email/threads.ts` ganhou `resolveOwnerOrders` (exportada, agora com `client_id`) e `resolveThreadsForOrderIds` (recebe Orders já resolvidos, não ids soltos — evita round-trip repetido); `resolveThreadsForSend` foi removida (só `sendStepEmail` a chamava, e ele precisa do array de Orders de qualquer forma). Cada grupo de idioma manda pras threads só dos SEUS Orders (via `client_id` de cada Order); Order órfão (cliente fora de `pre_loading_clients`, que é editado à mão — `app/(dashboard)/pre-loading/actions.ts` — e pode divergir dos Orders reais) cai no grupo primário; grupo sem Order nenhum (drift no sentido oposto) também cai no primário. A equipe interna continua sempre atingindo TODAS as threads, sem depender de grupo.
- **Colapsa pro código de hoje quando ≤1 grupo tem destinatário selecionado NESTE envio específico** — não "quantos idiomas o PL resolve" em abstrato. Isto foi uma correção em cima da primeira versão do desenho (revisão por um segundo agent, antes de codar): separar "linha do interno" de "linha do cliente" incondicionalmente mudaria até o envio comum de hoje (Responsible + 1 contato do cliente, 1 idioma só) de 1 linha pra 2, mudando o `status` computado. Com o gate, só 2+ grupos ativos de verdade viram mensagens/linhas separadas em `checklist_step_emails` (sem migration — tabela já não tem UNIQUE que impeça).
...
> ✅ **Terceiro limite alinhado (11/09/2026):** o bucket `order-documents` do Supabase Storage tinha `file_size_limit = 20.000.000` bytes (~19,07 MiB, número "redondo" em decimal), enquanto `MAX_FILE_BYTES` no código é `20 × 1024 × 1024 = 20.971.520` (20 MiB). Na janela entre os dois o app aceitava o arquivo e o Storage recusava com erro genérico, sem a mensagem amigável de tamanho. Bucket atualizado pra `20971520` via Storage API (`PUT /storage/v1/bucket/order-documents`). O bucket segue sem restrição de MIME (`allowed_mime_types: null`); o outro bucket, `business-units`, é só logo (5 MB, PNG/JPEG/SVG).

##### Upload de anexo NÃO passa mais pelo servidor — limite de 4,5MB da Vercel (redesenho, 11/09/2026)

A correção acima (`bodySizeLimit`/`proxyClientMaxBodySize`) foi testada só **local** e em produção não resolveu nada: o cliente testou com 4MB no dia seguinte e continuou quebrando. Reproduzido com `curl` contra `sot.gssdatahub.com`: corpo de 4MB entra (307 pro login, ou seja, chegou no app), corpo de 5MB volta **`413 Request Entity Too Large / FUNCTION_PAYLOAD_TOO_LARGE`** respondido pela própria Vercel (`gru1`), antes de qualquer código nosso. É o **limite fixo de 4,5MB por request das Serverless Functions da Vercel** — nenhum config do Next altera, e um "4MB" do Explorer costuma ser 4,2–4,4 MiB reais + boundaries do multipart, por isso estourava. Enquanto o arquivo viajasse dentro de uma Server Action, os 20MB prometidos eram inatingíveis.

**Redesenho: o arquivo vai direto do browser pro Supabase Storage**, e só metadados passam pelo servidor. Fluxo em 3 passos, igual nos 3 módulos (`lib/attachments*.ts` + `actions.ts` de Order/Pre-loading/Shipment):

1. `create*AttachmentTicket(…, fileName, fileSize)` — Server Action: checa permissão (`requireFeature` edit; no Shipment também `canEditInheritedStep`), valida `MAX_FILE_BYTES`, no PL/Shipment cria a linha da etapa se não existir (`ensureStepId`), e emite uma **URL assinada de upload** (`storage.createSignedUploadUrl`, token de 2h preso a um `path` exato: `<orderId>/<stepId>/<ts>-<nome>` ou `pre-loading/<plId>/<stepRowId>/…`).
2. Browser (`uploadDirect` em `lib/attachments-client.ts`): `storage.uploadToSignedUrl(path, token, file)` com a chave `anon` — a URL assinada dispensa RLS, o token autoriza só aquele objeto. Valida tamanho antes de pedir o ticket (toast imediato "File is larger than 20MB.").
3. `register*Attachment(…, path, fileName)` — Server Action: insere em `step_attachments`; se o insert falhar, apaga o objeto. **Confere que o `path` pertence ao prefixo da etapa** (`isPathInDir`) — sem isso um client mal-intencionado registraria qualquer objeto do bucket como anexo.

Teto efetivo agora é o **`file_size_limit` do bucket (20 MiB)**, que bate com `MAX_FILE_BYTES`. Os limites em `next.config.ts` ficaram (comentário reescrito) só como folga pra Server Actions comuns — não governam mais o upload. Testado local com o dev server: Order 8MB (`.xlsm`) e Pre-loading 6MB (`.pdf`) subiram via `fetch` direto pro `supabase.co` (~1,1–1,3s), objeto no bucket com o tamanho exato, linha registrada e listada na etapa; 21MB barrado no client com o toast. Objetos de teste apagados depois.
...
| "Forgot password" → e-mail com link | `admin.auth.admin.generateLink({ type: "recovery" })` + envio pelo Resend (ver 3.1) — **não** `resetPasswordForEmail()` |
...
- **RLS:** começou desligado (segurança na camada de aplicação) e depois foi habilitado em **deny-all** (fail closed) — some o aviso "UNRESTRICTED" do dashboard e fecha o acesso via chave `anon`. Acesso atual: só `service_role` (servidor / importador). **Policies por tabela ficam para a fase de RLS.**
  - ⚠️ **Gap encontrado 15/09/2026:** toda nova tabela criada por migration precisa do próprio `ENABLE ROW LEVEL SECURITY` — o deny-all de `20260727095000` só cobriu as 33 tabelas que já existiam naquela data. `country_language_defaults` (`20260909120000`) ficou sem, e o Supabase Security Advisor sinalizou (`rls_disabled_in_public`) em 13/09. Corrigido em `20260915120000_country_language_defaults_rls.sql`. Conferidas as outras 13 tabelas criadas depois do deny-all — todas as demais já tinham o `enable row level security` na própria migration de criação.
